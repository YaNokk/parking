...
export function Park ({ electronicCharge, lat, long, title,distance, commonFill, invalidFill, commonRemaining, invalidRemaining, tariff  }) {
    const isMobile = useMediaQuery({
        query: '(max-width: 600px)'
    })
    const isBigger = distance >= 1000;
    return <div className={style.park}>
        <div className={style.main} style={isMobile ? { flex : "1"} : {}}>
        ...
        <div className={style.statuses}>
            ...
            {electronicCharge ? <div className={style.geo}>
                <svg width="16px" height="16px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#000000" strokeWidth="1" strokeLinecap="round" strokeLinejoin="miter"><polyline points="11.25 7 2 7 2 17 11 17" strokeLinecap="round"></polyline><polyline points="16 7 20 7 20 17 16 17"></polyline><polyline points="12 6 8 12 14 12 10 19"></polyline><polyline points="20 10 22 10 22 14 20 14"></polyline></svg>
                <span className={style.status}>{'Зарядка'}</span>
            </div> : null}
        </div>
        ...
        {isMobile ? null : <div className={style.actions}>
            <a target={'_blank'} href={`https://yandex.ru/maps/?ll=${long},${lat}&z=15`}  className={classNames(style.buttonBorder, style.yellow)}>
                <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                    <path d="M16 13.2506V1.81191C16 1.45847 15.6112 1.24299 15.3115 1.43031L12.1847 3.38455C12.068 3.45747 11.9244 3.47301 11.7949 3.42674L5.20514 1.07326C5.07557 1.02699 4.93195 1.04253 4.81528 1.11545L1.2115 3.36781C1.07993 3.45004 1 3.59426 1 3.74941V15.1881C1 15.5415 1.38878 15.757 1.6885 15.5697L4.81528 13.6154C4.93195 13.5425 5.07557 13.527 5.20514 13.5733L11.7949 15.9267C11.9244 15.973 12.068 15.9575 12.1847 15.8846L15.7885 13.6322C15.9201 13.55 16 13.4057 16 13.2506Z" stroke="#B29600" strokeWidth="1.5"/>
                    <path d="M12 16V3.5" stroke="#B29600" strokeWidth="1.5"/>
...
function Indicator ({ name, remaining, fill}) {
    return <div className={style.indicator}>
        ...
        <div className={style.progress}>
            <div className={classNames(style.fill, fill <= 40 ? style.success : fill >= 40 && fill <= 80 ? style.common : style.danger)} style={{ width : `${fill}%`}}/>
        </div>
    </div>
}